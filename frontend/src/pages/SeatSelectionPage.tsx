...
import { ArrowLeft, Bus, AlertCircle, Clock, Loader2 } from 'lucide-react';
import { formatPrice, formatTime } from '../data/mockData';
import { useTrip, useSeats } from '../hooks/useApi';
...
export default function SeatSelectionPage() {
    ...
    // Fetch trip and seats from API
    const { data: tripInfo, isLoading: tripLoading, error: tripError } = useTrip(tripId || '');
    const { data: apiSeats = [], isLoading: seatsLoading } = useSeats(tripId || '');

    // Use API seats or empty array
    const seats: Seat[] = apiSeats;
        ...
        return (
            <div className="min-h-screen bg-gray-50 flex items-center justify-center">
                <div className="text-center">
                    ...
                    <p className="text-gray-600">Chargement des sièges disponibles...</p>
                </div>
            </div>
        );
    }
    ...
    return (
        <div className="min-h-screen bg-gray-50">
            {/* Header */}
            <header className="bg-primary-500 text-white py-4 sm:py-6">
                <div className="container mx-auto px-4">
                    <button
                        onClick={() => navigate(-1)}
                        className="flex items-center gap-2 text-white/90 hover:text-white mb-3 sm:mb-4 transition-colors"
                    >
                    ...
                    <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2 sm:gap-4">
                        <div>
                            <h1 className="text-xl sm:text-2xl font-bold">Choisissez vos sièges</h1>
                            <p className="text-white/80 text-sm sm:text-base">
                                {tripInfo.company.name} • {tripInfo.route.fromCity} → {tripInfo.route.toCity}
                            </p>
                        </div>
                        <div className="text-left sm:text-right">
                            <p className="text-xl sm:text-2xl font-bold">{formatTime(tripInfo.departureTime)}</p>
                            <p className="text-white/80 text-sm">Départ</p>
                        </div>
                    </div>
                </div>
            </header>
                ...
                <div className="grid lg:grid-cols-3 gap-4 sm:gap-8">
                    ...
                    <div className="lg:col-span-2">
                        <div className="card">
                            ...
                            {/* Bus Layout */}
                            <div className="bg-gradient-to-b from-gray-100 to-gray-50 rounded-2xl sm:rounded-3xl p-4 sm:p-6 max-w-md mx-auto">
                                {/* Driver area */}
